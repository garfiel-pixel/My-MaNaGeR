...
(async () => {
  ...
  await check('mcp3: MCP POST without auth -> generic 403', `(function(){return fetch('/api/mcp/qa-full-mcp',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'initialize',params:{}})}).then(function(r){return r.json().then(function(d){return {val:r.status===403 && d.ok===false};});});})()`);
