...
(async () => {
  ...
  await delay(120);
  await check('02 theme: light default + Appearance select reflects', `(function(){var s=document.getElementById('theme-select');return {val: !!s && s.value==='light' && !document.body.classList.contains('dark-mode')};})()`);
  await ev(`var _ts=document.getElementById('theme-select'); _ts.value='dark'; _ts.dispatchEvent(new Event('change',{bubbles:true}));`); await delay(200);
  await check('02b theme: select dark -> class + device pref', `(function(){var s=document.getElementById('theme-select');return {val: !!s && s.value==='dark' && document.body.classList.contains('dark-mode') && localStorage.getItem('mmgr_theme') === 'dark' };})()`);
  await ev(`var _ts=document.getElementById('theme-select'); _ts.value='light'; _ts.dispatchEvent(new Event('change',{bubbles:true}));`); await delay(200);
  ...
  // ---- MCP TRANSPORT - standing regression gates (owner 2026-09-16) ----
  // The /api/mcp/:id transport contract against the dev-server mirror
  // (serve.cjs reuses the REAL refusal text from src/mcp/server.js so gates
  // assert identical bytes). Deep auth/D1 coverage lives in
  // tools/qa-api-keys.cjs (real wrangler + D1); the rate-limit (audit F1)
  // gates live in tools/qa-ai-relay.cjs against the real router module.
  await check('mcp1: dev mint returns a shown-once sk-mmgr- key (scoped)', `(function(){return fetch('/__qa/mcp-reset',{method:'POST'}).then(function(){return fetch('/api/cloud/projects/qa-full-mcp/api-keys',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label:'qa-full',scope:['wbs']})});}).then(function(r){return r.json();}).then(function(d){return {val: d.ok===true && typeof d.apiKey==='string' && d.apiKey.lastIndexOf('sk-mmgr-',0)===0 && d.scope.join(',')==='wbs'};});})()`);
  await check('mcp2: MCP GET -> 405 JSON "MCP server requires POST"', `(function(){return fetch('/api/mcp/qa-full-mcp').then(function(r){return r.json().then(function(d){return {val:r.status===405 && d.ok===false && String(d.error).indexOf('requires POST')!==-1};});});})()`);
  await check('mcp3: PATH-A - unauthenticated POST initialize -> 200 JSON-RPC (transport never 401s)', `(function(){return fetch('/api/mcp/qa-full-mcp',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'initialize',params:{}})}).then(function(r){return r.json().then(function(d){return {val:r.status===200 && !!(d.result&&d.result.serverInfo&&d.result.serverInfo.name==='my-manager-mcp')};});});})()`);
  await check('mcp7: PATH-A - unauthenticated tools/call -> 200 isError credential refusal, no data', `(function(){return fetch('/api/mcp/qa-full-mcp',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:8,method:'tools/call',params:{name:'get_tasks',arguments:{}}})}).then(function(r){return r.json().then(function(d){var t=((d.result||{}).content||[])[0]||{};return {val:r.status===200 && d.result && d.result.isError===true && String(t.text||'').indexOf('needs its own credential')!==-1 && String(t.text||'').indexOf('Pour foundation')===-1};});});})()`);
  await check('mcp4: MCP initialize with Bearer sk-mmgr- AND X-API-Key -> serverInfo', `(function(){return fetch('/__qa/mcp-reset',{method:'POST'}).then(function(){return fetch('/__qa/mcp-seed',{method:'POST'});}).then(function(){return fetch('/api/cloud/projects/qa-full-mcp/api-keys',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label:'qa-full',scope:['wbs','risk']})});}).then(function(r){return r.json();}).then(function(d){var k=d.apiKey;return Promise.all([fetch('/api/mcp/qa-full-mcp',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+k},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'initialize',params:{}})}).then(function(r){return r.json();}),fetch('/api/mcp/qa-full-mcp',{method:'POST',headers:{'Content-Type':'application/json','X-API-Key':k},body:JSON.stringify({jsonrpc:'2.0',id:2,method:'initialize',params:{}})}).then(function(r){return r.json();})]);}).then(function(rs){return {val: !!(rs[0].result&&rs[0].result.serverInfo&&rs[0].result.serverInfo.name==='my-manager-mcp') && !!(rs[1].result&&rs[1].result.serverInfo)};});})()`);
  await check('mcp5: scoped key - get_risks refused with exact text, get_tasks returns seeded data', `(function(){return fetch('/__qa/mcp-reset',{method:'POST'}).then(function(){return fetch('/__qa/mcp-seed',{method:'POST'});}).then(function(){return fetch('/api/cloud/projects/qa-full-mcp/api-keys',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label:'qa-full',scope:['wbs']})});}).then(function(r){return r.json();}).then(function(d){var k=d.apiKey;var H={'Content-Type':'application/json','X-API-Key':k};var call=function(nm){return fetch('/api/mcp/qa-full-mcp',{method:'POST',headers:H,body:JSON.stringify({jsonrpc:'2.0',id:9,method:'tools/call',params:{name:nm,arguments:{}}})}).then(function(r){return r.json();});};return Promise.all([call('get_risks'),call('get_tasks')]);}).then(function(rs){var ref=((rs[0].result||{}).content||[])[0]||{};var okd=((rs[1].result||{}).content||[])[0]||{};return {val: rs[0].result&&rs[0].result.isError===true&&String(ref.text||'').indexOf('does not include that section')!==-1&&String(ref.text||'').indexOf('Rain delay')===-1&&rs[1].result&&!rs[1].result.isError&&String(okd.text||'').indexOf('Pour foundation')!==-1};});})()`);
  await check('mcp6: tools/list carries the tool surface; unknown method -> JSON-RPC -32601', `(function(){return fetch('/__qa/mcp-reset',{method:'POST'}).then(function(){return fetch('/api/cloud/projects/qa-full-mcp/api-keys',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label:'qa-full',scope:['wbs']})});}).then(function(r){return r.json();}).then(function(d){var k=d.apiKey;var H={'Content-Type':'application/json','Authorization':'Bearer '+k};return Promise.all([fetch('/api/mcp/qa-full-mcp',{method:'POST',headers:H,body:JSON.stringify({jsonrpc:'2.0',id:3,method:'tools/list'})}).then(function(r){return r.json();}),fetch('/api/mcp/qa-full-mcp',{method:'POST',headers:H,body:JSON.stringify({jsonrpc:'2.0',id:4,method:'no/such'})}).then(function(r){return r.json();})]);}).then(function(rs){var names=((rs[0].result||{}).tools||[]).map(function(t){return t.name;});return {val: names.indexOf('get_tasks')>-1&&names.indexOf('apply_changes')>-1&&!!(rs[1].error&&rs[1].error.code===-32601)};});})()`);
