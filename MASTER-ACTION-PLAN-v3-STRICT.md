...
**CROSS-RANK DEPENDENCY (read this before executing top-to-bottom):** Rank 1 cannot
be verified complete until Rank 7.4/7.5 (daily weather log, LD field) exist — build
those two items early, even though Rank 7 as a whole comes later. Rank 1's claim-pack
acceptance test pulls weather-delay rows and the LD rate from state; without them the
package cannot be populated end-to-end. Do not declare Rank 1 done against an empty
weather log.
