...
**2026-08-31 — Session: UI POLISH ROUND 3 — cloud card menu, glass polish, dark theme contrast, carousel restart, favicon standardization, field guide sign-in fix.**
(1) **Cloud card 3-dot menu hover visibility** (css/mmgr.css): the 3-dot delete/options menu on cloud project cards was always visible. Now hidden by default (`opacity:0;pointer-events:none`) and revealed on card hover/focus (`cd-card:hover .cd-menu`). The delete option remains behind the trigger menu, never directly visible. Token-driven, dark-mode overrides unchanged. (2) **Dark theme glass polish** (css/mmgr.css body.dark-mode): glass-blur reduced 20px → 12px, glass-saturate 180% → 140%, glass-fill-dark 0.08 → 0.05, rim 0.15 → 0.08, border 0.10 → 0.07, shadow 0.35 → 0.25. The glass is now a subtle complement rather than an overpowering gloss. (3) **Marketing dark theme dropdown contrast** (css/marketing.css): added `body.dark-mode .nav-dd-item` rules ensuring dropdown items are always readable against the panel background in dark mode (text uses --text-primary, hover uses --gold with 12% opacity fill). (4) **Marketing carousel restart on return** (js/marketing.js): added IntersectionObserver on the .features track — when the feature bar scrolls back into view after being hidden, the auto-tick timer restarts automatically. Previously the tick only paused/resumed on hover/visibilitychange but not on viewport re-entry. (5) **Favicon standardization** (admin.html, project.html, app.html): all three pages now use `primary-icon.png` instead of `icon.svg`. index.html kept its gold-spark SVG identity per the owner's directive. (6) **Field guide sign-in fix** (mymanager-field-guide.html, css/marketing.css): the sign-in button was hidden by default (`display:none!important`) and only showed when signed in, preventing users from clicking it to sign in. Fixed: `.site-signin.signin-trigger` elements are now always visible (`display:inline-flex!important`) while generic `.signin-btn` marketing CTAs remain dormant. The field guide sign-in is a simple sidebar action, not a marketing CTA. (7) **Admin panel sidebar already has user chip + premium + theme toggle** (verified from code — admin.html rail has Account group with auth-bar + plan-pill + Customize group with dark/theme/palette controls matching app.html). (8) **Admin sidebar dark theme already matches app page** (verified — css/mmgr.css has `body.dark-mode.db-page` overrides for every admin rail element). VERIFICATION: `npm run verify` GREEN (CSP 17/17, SW v217 > 23 assets, hidden 15 pages, skills 17/17, exports all pass); qa-marketing **20/20**; qa-dashboard-spec **77/77**; qa-changelog-diffs **all gates passed**; build clean (6 bundles + 2 CSS). REMAINING: mobile view-only clutter fix (needs browser inspection of the actual viewport behavior), local sign-in option visibility when signed in via email (needs code audit of the sign-in sheet logic), project settings controls dropdown/accordion restructuring (CSS + JS collapsible sections). NEXT: continue with remaining UI fixes per owner's directive.
