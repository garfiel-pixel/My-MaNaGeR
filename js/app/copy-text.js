...
(function(ns) {
  ...
  function buildDigest(s) {
    ...
    var risks = (s.risks || []).filter(function(r) { return !r.issueId && (r.probability === 'High' || r.probability === 'high'); });
    if (risks.length) lines.push('High risks: ' + risks.map(function(r) { return r.description; }).join('; '));
    var issues = (s.issues || []).filter(function(i) { return i.status !== 'resolved' && i.status !== 'closed'; });
    if (issues.length) lines.push('Open issues: ' + issues.map(function(i) { return i.description; }).join('; '));
    var planned = (s.budgetLines || []).reduce(function(n, l) { return n + (+l.planned || 0); }, 0);
    var actual = (s.budgetLines || []).reduce(function(n, l) { return n + (+l.actual || 0); }, 0);
  ...
  function copyAsText(kind) {
    ...
    if (kind === 'slack') {
      ...
      lines.push('Open issues: ' + (s.issues || []).filter(function(i) { return i.status !== 'resolved' && i.status !== 'closed'; }).length);
      lines.push('Open high risks: ' + (s.risks || []).filter(function(r) { return !r.issueId && (r.probability === 'High' || r.probability === 'high'); }).length);
      lines.push('');
  ...
  function emailTplText(kind) {
    ...
    var openIssues = (s.issues || []).filter(function(i) { return i.status !== 'resolved' && i.status !== 'closed'; });
    var body = '';
    if (kind === 'status') {
      ...
      var pending = (s.changes || []).filter(function(c) { return c.status === 'submitted' || c.status === 'review'; });
      ...
      var highRisks = (s.risks || []).filter(function(r) { return !r.issueId && (r.probability === 'High' || r.probability === 'Very High' || r.impact === 'High' || r.impact === 'Very High'); });
