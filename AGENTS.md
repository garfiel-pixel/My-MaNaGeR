...
2. **Deploy from a clean staging copy.** `wrangler.jsonc`'s assets directory is    `.`, and wrangler does NOT honor gitignore for assets - uploads bloated and
...
4. **`skills-lock.json` is the source of truth** for installed skills
   ...
   computed via `tools/hash-skill-folder.cjs` (same algorithm as the verify script).    Skills run with full agent permissions - review before use.
...
7. **NO EMOJI ON ANY SERVED PAGE - HARD GATE (owner, 2026-08-13).** Zero emoji
   ...
   explicit inline `<svg>` on standalone pages, the monolith's inlined sprite.    The "higher form of SVG" = one shared symbol sprite - never per-icon
...
8. **NO AI-ASSISTANT CREDIT IN COMMITS - HARD GATE (owner).** The owner
   ...
   Commit messages must NEVER carry a `Generated with …` or    `Co-Authored-By: …` footer crediting any AI assistant, tool, or bot - no
...
2. **Reproduce locally before touching code.** If the suite is
   self-contained (starts its own wrangler/Chrome) run it directly:    `node tools/<suite>.cjs`. T2 suites need no server - they spawn
...
3. **Fix the HARNESS when the app changed under it, not the app.** Every
   ...
   unlocks the admin gate via `adminSetupPassword` and expects rows    immediately - Phase 2's show-once recovery modal (#rc-om) parks the
...
6. **Record the fix in the tracker** (PLANNING-TODO-2026-09-03.txt) -    commit message + which harness/app contract changed - so future waves
